            ...
            <div class="box-content">
                <form class="form-inline">
                    <div class="form-group">
                        <input type="text" class="form-control" id="mm_emp_nickname" value="${query.mm_emp_nickname}"
                               placeholder="名称">
                    </div>
                    <div class="form-group">
                        <input type="tel" class="form-control" id="mm_emp_mobile" value="${query.mm_emp_mobile}"
                               placeholder="电话">
                    </div>
                    <div class="form-group">
                        <select class="form-control" id="order_status">
                            <option value="">--选择订单状态--</option>
                            <option value="1" ${query.status=='1'?'selected':''}>订单生成</option>
                            <option value="2" ${query.status=='2'?'selected':''}>订单支付</option>
                            <option value="3" ${query.status=='3'?'selected':''}>订单取消</option>
                            <option value="4" ${query.status=='4'?'selected':''}>订单作废</option>
                        </select>
                    </div>
                    <div class="form-group">
                        <select class="form-control" id="pay_status">
                            <option value="">--选择付款状态--</option>
                            <option value="0" ${query.pay_status=='0'?'selected':''}>未支付</option>
                            <option value="1" ${query.pay_status=='1'?'selected':''}>已支付</option>
                            <option value="2" ${query.pay_status=='2'?'selected':''}>退款</option>
                        </select>
                    </div>
...
<script type="text/javascript">
    function searchIndex(e) {
        ...
        if (_index <= ${page.pageCount} && _index >= 1) {
            window.location.href = "#module=/order/list&page=" + _index + "&size=" + size +
