...
public class GoodsCommentService implements SaveService, ListService {
    ...
    public Object save(Object object) throws ServiceException {
        ...
        EmpVO member = memberDao.findById(comment.getEmpId());
        ...
        if (!StringUtil.isNullOrEmpty(comment.getFplid()) && !"".equals(comment.getFplid()) && !"0".equals(comment.getFplid())){
            ...
            if (!comment.getEmpId().equals(comment.getGoodsEmpId())){
                ...
                relate.setCont(member.getMm_emp_nickname()+" 评论了你的商品");
            ...
            if (!comment.getFplempid().equals(comment.getGoodsEmpId()) && !comment.getFplempid().equals(comment.getEmpId())){
                ...
                relate.setCont(member.getMm_emp_nickname()+" 回复了你的商品评论");
                relateDao.save(relate);
            }
        ...
        EmpVO pushMember =  memberDao.findById(comment.getGoodsEmpId());
        ...
        if (!StringUtil.isNullOrEmpty(comment.getFplid()) && !"".equals(comment.getFplid()) && !"0".equals(comment.getFplid())){
            ...
            if (!comment.getGoodsEmpId().equals(comment.getEmpId())) {
                //动态所有者发通知
                pushZan(pushId,type, member.getMm_emp_nickname() + " 评论了你的商品");
            }

            if (!comment.getFplempid().equals(member.getMm_emp_id()) && !comment.getFplempid().equals(comment.getGoodsEmpId())) {
                //向父评论者发通知
                EmpVO pushMember1 =  memberDao.findById(comment.getFplempid());
                String pushId1 =pushMember1.getPushId();
                String type1 = pushMember1.getDeviceType();

                pushZan(pushId1, type1, member.getMm_emp_nickname() + " 回复了你的商品评论");
            }
        }else {
            //没有父评论
            if (!comment.getGoodsEmpId().equals(comment.getEmpId())) {
