...
public class CommentService implements SaveService, ListService {
    ...
    public Object save(Object object) throws ServiceException {
        ...
        commentDao.save(comment);
         ...
         Emp member = memberDao.findById(comment.getEmpId());
        if (!StringUtil.isNullOrEmpty(comment.getFplid()) && !"".equals(comment.getFplid()) && !"0".equals(comment.getFplid())){
           if (!comment.getEmpId().equals(comment.getSendEmpId())){
               ...
               relate.setCont(member.getMm_emp_nickname()+" 评论了你的动态");
        ...
        if (!StringUtil.isNullOrEmpty(comment.getFplempid()) && !comment.getFplempid().equals(comment.getSendEmpId()) && !comment.getFplempid().equals(comment.getEmpId())){
            ...
            relate1.setCont(member.getMm_emp_nickname()+" 回复了你的评论");
        ...
        Emp pushMember =  memberDao.findById(comment.getSendEmpId());
        ...
        if (!StringUtil.isNullOrEmpty(comment.getFplid()) && !"".equals(comment.getFplid()) && !"0".equals(comment.getFplid())){
            ...
            if (!comment.getSendEmpId().equals(comment.getEmpId())) {
                //动态所有者发通知
                pushZan(pushId,type, member.getMm_emp_nickname() + " 评论了你的动态");
            }

            if (!comment.getFplempid().equals(member.getMm_emp_id()) && !comment.getFplempid().equals(comment.getSendEmpId())) {
                //向父评论者发通知
                Emp pushMember1 =  memberDao.findById(comment.getFplempid());
                String pushId1 =pushMember1.getPushId();
                String type1 = pushMember1.getDeviceType();
                pushZan(pushId1, type1, member.getMm_emp_nickname() + " 回复了你的评论");
            }
        }else {
            //没有父评论
            if (!comment.getSendEmpId().equals(comment.getEmpId())) {
